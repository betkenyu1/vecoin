...
function getListaClientes() {
  ...
  html += '<th class="text-nowrap">% Retencion I.R.</th>';
  html += '<th class="text-nowrap">% Retencion I.V.A.</th>';
  ...
  $.ajax({
    ...
    success: function (response) {
      $.each(response, function (key, value) {
        ...
        html += "<td>" + value.porc_ret_renta + " %" + "</td>";
        html += "<td>" + value.porc_ret_iva + " %" + "</td>";
        html += "<td>" + value.id_estado + "</td>";
        html += "<td>";
        html +=
          '<a class="btn btn-outline-warning" onclick="setModificarCliente(' +
          value.id_cliente +
...
function setCliente() {
  ...
  html +=
    '<input type="text" name="txt" placeholder="Ingrese R.U.C." minlength="13"  onkeypress="return validarRUC(event);" class="form-control" id="IdRuc">';
  ...
  html +=
    '<input type="text" onkeypress="return validarCorrecion(event)" placeholder="Ingrese Plazo de pago" class="form-control" id="IdTiempocredito">';
  ...
  html +=
    '<select class="default-select2 form-control" name="IdPorRenta" id="IdPorRenta"></select>';
  ...
  html +=
    '<select class="default-select2 form-control" name="IdPorIVA" id="IdPorIVA"></select>';
...
function getGuardarCliente() {
  ...
  if (
    ...
    var pir = (Math.round($("#IdPorRenta").val() * 100) / 100).toFixed(2);
    var piva = (Math.round($("#IdPorIVA").val() * 100) / 100).toFixed(2);
    Swal.fire({
...
function setModificarCliente(id_cliente) {
  ...
  html += '<b style="color: #000000;">R.U.C.:</b> </br>';
  html +=
    '<input type="text" name="txt" placeholder="Ingrese R.U.C." minlength="13"  onkeypress="return validarRUCMod(event);" class="form-control" id="IdRuc_mod">';
  html += '<div id="alert-rc"></div>';
  html += "</div>";
  html += "</div>";
  ...
  html += '<b style="color: #000000;">Teléfono:</b> </br>';
  html +=
    '<input type="text" maxlength="10" onkeypress="return validarTelefonoMod(event)" placeholder="Ingrese Teléfono" class="form-control" id="IdTelefono_mod">';
  html += '<div id="alert-tl"></div>';
  html += "</div>";
  html += "</div>";
  ...
  html += '<b style="color: #000000;">Plazo de pago:</b> </br>';
  html +=
    '<input type="text" onkeypress="return validarCorrecion(event)" placeholder="Ingrese Plazo de pago" class="form-control" id="IdTiempocredito_mod">';
  html += '<div id="alert-pp"></div>';
  html += "</div>";
  html += "</div>";
  ...
  html += '<div class="text-center">';
  html +=
    '<a class="btn btn-outline-danger" onclick="CerrarModificarCliente();" title="Cerrar"><i class="fa-solid fa-cancel" aria-hidden="true"></i> Cerrar</a>';
  html +=
    '&nbsp;<a class="btn btn-outline-primary" title="Modificar" onclick="getModificarCliente();"><i class="fa-solid fa-pencil" aria-hidden="true"></i> Modificar</a>';
  html += "</div>";
