...
function validarPrecio(evt) {
  ...
  if ($("#IdPVP").val().length < 1 || $("#IdPVP").val() != "") {
    if (code == 8) {
      // backspace.
      return true;
  ...
  $.ajax({
    ...
    success: function (response) {
      $.each(response, function (key, value) {
        ...
        html += "<td>";
        html +=
          '<a class="btn btn-outline-danger" onclick="getReporteOrdenSalida(' +
          value.id_secuencial +
...
function setNuevaOrdenSalida() {
  ...
  html +=
    '<input type="text" class="form-control" onkeypress="return validarEnteros(event)" placeholder="Cantidad a despachar" id="IdCantidad">';
  ...
  html += "<br>";
  html +=
    '<a class="btn btn-danger" onclick="CerrarNuevaOrdenSalida();" title="Cancelar"><i class="fa-solid fa-cancel" aria-hidden="true"></i> Cancelar</a>';
  html +=
    '&nbsp;<a class="btn btn-primary" title="Agregar" onclick="getAgregarOrdenSalida();"><i class="fa-solid fa-plus" aria-hidden="true"></i> Agregar</a>';
  html +=
    '&nbsp;<a class="btn btn-success" title="Cerrar" onclick="getCerrarOrdenSalida();"><i class="fa-solid fa-save" aria-hidden="true"></i> Cerrar</a>';
  html += "</div>";
  html += "</div>";
...
function getAgregarOrdenSalida() {
  ...
  if ($("#IdCantidad").val() == "" || $("#IdCantidad").val() == 0) {
    ...
    validarDisponibilidad($("#IdCantidad").val(), $("#IdExistencia").val()) ==
    false
  ) {
    html += '<div class="alert alert-danger">';
